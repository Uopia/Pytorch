...
class ModelEvaluation:
                ...
                for res_coo in res_coos:
                    if res_coo[-1] == b_label:
                        ...
                        img_cvt_c = cv2.putText(img_cvt_c, res_coo[-1], (int(min(res_coo[0], res_coo[2])) + 10,
                                                ...
                                                font, 0.8, (0, 255, 255), 3)
                        f.write(
                            f"{self.labels_list.index(res_coo[-1])} {(res_coo[0] + res_coo[2]) / 2 / w} {(res_coo[1] + res_coo[3]) / 2 / h} {abs(res_coo[0] - res_coo[2]) / w} {abs(res_coo[1] - res_coo[3]) / h}\n")
                ...
                for lab_coo in lab_coos:
                    if lab_coo[-1] == b_label:
                        ...
                        img_cvt_cc = cv2.putText(img_cvt_cc, lab_coo[-1], (int(w * (lab_coo[0] - lab_coo[2] / 2)) + 10,
                                                 ...
                                                 font, 0.8, (255, 255, 0), 3)
                        f.write(
                            f"{self.labels_list.index(lab_coo[-1])} {lab_coo[0]} {lab_coo[1]} {lab_coo[2]} {lab_coo[3]}\n")
    ...
    def output_log(self):
        ...
        f.write("预测对了，预测错了\n")
        f.write(f"标签总数，{self.sum[0][0]+self.sum[0][1]+self.sum[1][0]}\n")
        for name in self.labels_list:
            f.write(f"{self.labels_list.index(name)}、{name}:召回率={self.labels[name][2][0]}，准确率={self.labels[name][2][1]}\n")
