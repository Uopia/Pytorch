...
LabelWeightDict = {"RB":1.00,"OB":0.5518,"PF":0.2896,"DE":0.1622,"FS":0.6419,"IS":0.1847,"RO":0.3559,"IN":0.3131,
                   "AF":0.0811,"BE":0.2275,"FO":0.2477,"GR":0.0901,"PH":0.4167,"PB":0.4167,"OS":0.9009,"OP":0.3829,
                   "OK":0.4396}
...
def calcualteResults(args):
    ...
    for subdir, dirs, files in os.walk(scorePath):
        print(subdir)
        print(files)  # 为了查看文件夹中的文件列表
        for scoreFile in files:
            print(scoreFile)  # 查看正在处理的文件名
            ...
            aveargeString = "{:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f}".\
                format(main["mF1"]*100, main["MF1"]*100, main["OF1"]*100, main["OP"]*100, main["OR"]*100, main["CF1"]*100, main["CP"]*100, main["CR"]*100,
                       main["EMAcc"]*100, main["mAP"]*100)
            ...
            print("New metrics:", newString)
            print("ML main metrics:", aveargeString)
            print("Class F1:", classF1String)
